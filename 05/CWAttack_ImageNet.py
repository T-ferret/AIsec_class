...
def main():
    ...
    # Targeted
    target_labels = torch.tensor([3, 9, 100, 500, 900], dtype=torch.long).to(device)
    print(target_labels)

    cw = torchattacks.CW(model, c=1, kappa=0, steps=10000, lr=0.001)  # 적대적(기만) 공격 방법 정의
    cw.set_normalization_used(mean=preprocess.mean, std=preprocess.std)
    cw.set_mode_targeted_by_label()
    print(cw)  # 공격 설정 출력
    # adv_exams = cw(org_imgs, y_test)
    adv_exams = cw(org_imgs, target_labels)
