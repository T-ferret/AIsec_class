...
def main():
    ...
    fgsm = torchattacks.FGSM(model, eps=8/255)  # 적대적(기만) 공격 방법 정의
    fgsm.set_normalization_used(mean=preprocess.mean, std=preprocess.std)
    print(fgsm)  # 공격 설정 출력
    adv_exams = fgsm(org_imgs, y_test)
