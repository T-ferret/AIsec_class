...
def main():
    ...
    mim = torchattacks.MIFGSM(model, eps=8/255, alpha=2/255, steps=20, decay=1.0)  # 적대적(기만) 공격 방법 정의
    mim.set_normalization_used(mean=preprocess.mean, std=preprocess.std)
    print(mim)  # 공격 설정 출력
    adv_exams = mim(org_imgs, y_test)
